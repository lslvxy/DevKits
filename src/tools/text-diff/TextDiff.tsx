...
export function TextDiffTool() {
  ...
  const compute = useCallback(() => {
    ...
    const patch = createTwoFilesPatch("原文", "新文", left, right);
    ...
    <div className="flex flex-col gap-4 p-6 h-full overflow-auto">
      <div className="grid grid-cols-2 gap-4">
        <div className="bg-[#252526] rounded-lg p-4 border border-[#3e3e42]">
          <h3 className="mb-3 text-sm font-medium text-[#d4d4d4]">原文</h3>
          <textarea
            value={left}
        ...
        <div className="bg-[#252526] rounded-lg p-4 border border-[#3e3e42]">
          <h3 className="mb-3 text-sm font-medium text-[#d4d4d4]">新文</h3>
          <textarea
            value={right}
      ...
      {(left || right) && (
        <div className="bg-[#252526] rounded-lg p-4 border border-[#3e3e42]">
          <h3 className="mb-3 text-sm font-medium text-[#d4d4d4]">差异结果</h3>
          <div className="max-h-96 overflow-auto rounded border border-[#3e3e42] bg-[#1e1e1e]">
            {!hasChanges(lines) ? (
              <p className="p-3 text-sm text-[#858585]">无差异</p>
            ) : (
              lines.map((line) => (
                <div
                  key={line.id}
